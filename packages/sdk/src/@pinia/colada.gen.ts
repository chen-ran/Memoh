...
export const getModelsMemoryQueryKey = (options?: Options<GetModelsMemoryData>) => createQueryKey('getModelsMemory', options);

/**
 * List memory-compatible models
 *
 * Get chat models supported by memory LLM pipeline
 */
export const getModelsMemoryQuery = defineQueryOptions((options?: Options<GetModelsMemoryData>) => ({
    key: getModelsMemoryQueryKey(options),
    query: async (context) => {
        const { data } = await getModelsMemory({
            ...options,
            ...context,
            throwOnError: true
        });
